...
def is_prime(number):
    if type(number) != int:
        raise TypeError("The input is not an integer")  # Correction de l'exception
    elif number < 2:
        return False
    else:
        for divider in range(2, number):
            if number % divider == 0:
                return False  # Retourne False si le nombre n'est pas premier
        
        return True  # Retourne True si le nombre est premier
print (is_prime(44))

# Exemple de boucle for supplémentaire
for num in range(0, 100):
    print(num)
    if num == 42:  # Condition d'arrêt arbitraire
        print("Nous avons trouvé le nombre 42, arrêt de la boucle.")
        break
else:
    print("Nous avons réussi à finir la boucle for sans interruption.")
